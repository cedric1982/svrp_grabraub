...
AddEventHandler("ricx_grave_robbery:check_shovel", function(id, Town)
    ...
    if item then
        if not next(item.metadata) then
            ...
            local newData = {
                description = "Schaufel Haltbarkeit %" .. 100 - 3,
                durability = 100 - 3,
                id = item.id
            }
            ...
            if item.metadata.durability <= 0 then
                TriggerClientEvent("Notification:left_grave_robbery", _source, TEXTS.GraveRobbery, "Schaufel ist gebrochen",
                    TEXTURES.alert[1], TEXTURES.alert[2], 2000)
                return
            end

            local newData = {
                description = "Schaufel Haltbarkeit %" .. item.metadata.durability - 3,
...
local Lines = {
    "Du hast nichts gefunden. Die hier begrabene Person war arm.",
    "All diese harte Arbeit umsonst, verdammter Idiot.",
    "Da könnte ich ja glatt Bauer werden, nur Graben und nichts finden.",
    "Du dachtest, es wäre einfach? Jemanden lebendig ausrauben geht schneller.",
    "Gott beobachtet dich und wird dich bestrafen. Genau wie er den Mann hier drin bestraft hat. Du bist der nächste ..."
